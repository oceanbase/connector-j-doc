...
| socketTimeout | The network socket timeout period (`SO_TIMEOUT`), in ms. The value `0` specifies to disable socket timeout. You can also set the `max_statement_time` system variable to limit the query time.  Default value: `0` (standard) or `10000`.  |
