...
| sendConnectionAttributes | Specifies whether to send extended attributes when a connection is created to provide more options to users with different permissions. Default value: `true`.  |
...
| useUnicode | Specifies whether to use the Unicode encodings. Default value: `true`. <br>**Note**This parameter cannot be modified in the current OceanBase Connector/J version  |
...
| enabledSslProtocolSuites | Specifies the TLS versions of the TLS/SSL protocol that must be used. Separate multiple versions with commas (,). Example: "TLSv1,TLSv1.1,TLSv1.2". The old alias is `enabledSSLProtocolSuites`. By default, Java values are used.  |
...
| Parameter | Description |
|--------------------|------------------|
| useLocalSessionState | Specifies whether the driver uses the internal values for automatic commit (specified by `Connection.setAutoCommit()`) and transaction isolation (specified by `Connection.setTransactionIsolation()`) and the transaction status maintained by the reference protocol.  Default value: `true`.<br>  **Note**This parameter cannot be modified in the current OceanBase Connector/J version.  |
| useLocalTransactionState | Specifies whether the driver uses the in-transaction status provided by the MySQL protocol to determine whether `commit()` or `rollback()` needs to be sent to the database.  Default value: `true`.<br>  **Note**This parameter cannot be modified in the current OceanBase Connector/J version.  |
| useArrayBinding | Specifies whether to concatenate parameters of the same types in each column into arrays when the PS protocol is used, so that several arrayBinding packets are sent at a time to improve the efficiency of data insertion. Default value: `false`.<br>  **Note**This parameter takes effect only in Oracle mode when `useServerPrepStmts` is set to true, `executeBatch()` or `executeLargeBatch()` is used, and the autocommit mode is disabled.</br>  |
