...
* Added ArrayBinding support in Oracle mode. When used under the PS/II protocol, it binds parameters as arrays to achieve multiple batch executions in one go, reducing data interaction costs between the server and client.
...
* Fixed an issue where, when useServerPrepStmts=true, if createStatement()->excuteQuery() failed, the case statement calling s.close() did not execute the close operation, triggering an error due to the observer ps cursor exceeding limits.
