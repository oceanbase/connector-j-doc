...
* Added ArrayBinding in Oracle mode. When used under the PS protocol, it binds parameters as arrays to achieve multiple batch executions in one go, reducing data interaction costs between the server and client.
...
* Fixed an issue where, when useServerPrepStmts=true, if createStatement()->excuteQuery() failed, calling s.close() in a case block did not actually close the statement, triggering an error due to the observer PS cursor exceeding its limit.
