...
- Fixed the error when specifying the Clob column's class as String during the call to `getObject()`.
- Fixed the error when setting `usePipelineAuth=true`.
- Fixed the error related to the `rewriteBatch` parameter.
- Fixed the issue where setting the port on OceanBaseDataSource did not take effect.
