...
| connectProxy            | 配置连接到 ODP。此时不能执行对业务 SQL 的查询，只能对 ODP 进行配置或者配置查询。默认值：`false`。 |
| obProxySocket           | 启用富客户端功能。默认值：""。|
| enableOb20Checksum      | 基于 OB2.0 协议，控制 OceanBase Connector/J 请求包的头部校验和、尾部校验和。默认值为 `ture`，表示计算 CRC 校验和；否则校验和为 0。|
| ocpAccessInterval       | 访问 OCP 的间隔时间（单位：分钟）。默认值：5。|
| httpConnectTimeout      | 设置指定的超时值（单位：毫秒），以在打开指向此 URLConnection 引用的网络资源的通信链接时使用。默认值：0。|
| httpReadTimeout         | 当 URLConnection 建立到资源时，非零值指定从输入流读取时的超时时间（单位：毫秒）。默认值：0。|
| compatibleOjdbcVersion  | 设置兼容目标 ojdbc 的版本，可设置为 6、8。设置为其它整数时不会报错，会按照默认值处理。默认值 6。 |  
| useOraclePrepareExecute | 开启 PS 二合一协议，Oracle 模式 `preparedStatement` 在使用 `COM_STMT_PREPARE_EXECUTE` 执行之前不会与 server 通信。默认值：`false`。 <br> 当前版本 `useOraclePrepareExecute` 为 `true` 时，`useServerPrepStmts` 被设为 `true`。V2.4.5 之前版本需要共同使用，才能开启 PS 二合一协议。 |
| compatibleMysqlVersion  | 设置兼容目标 mysql-jdbc 的版本，可设置为 5、8。设置为其它整数时不会报错，按照默认值处理。默认值 5。|
| mapDateToTimeStamp    | 设置为 `true` 时，Oracle 模式下 Date 数据类型将包含时分秒信息；设置为 `false` 时，则只显示日期部分。默认值 `true`。|
| obDateTypeOptimization  | 对于 `getDate` 方法使用从 Byte 数组获取值，而非转为字符串在进行截取。默认值 `false`。|
| useNewResultSetMetaData | 元数据信息兼容原生 MySQL JDBC。默认值 `false`。|
| usePieceData            | 设置是否使用分片数据传输模式。默认值：false。Oracle 模式下，使用 `COM_STMT_SEND_PIECE_DATA` 协议设置 `InputStream` 和 `Reader` 参数。当 `usePieceData` 为 true 时，`useOraclePrepareExecute` 被设为 true，`useCursorFetch` 被改为 true。          |
| useArrayBinding         | 支持 Oracle 模式下使用数组绑定来减少网络往返并提高性能。默认值：false。当此参数设置为 true 时，可以启用数组绑定，即将多个参数作为数组传递。  |
...
| useCompatibleMetadata         | 为 true 时，强制返回 MySQL/Oracle 作为数据库名，而不是 OceanBase。替代 useMysqlMetadata 选项。    |
...
| cachePrepStmts                          | 如果 `useServerPrepStmts = true`，则将准备好的信息缓存在 LRU 缓存中，以避免重新准备命令。下次使用该命令时，会将准备好的标识符和参数（如果有）发送到服务器，因而可以避免服务器重新解析查询。 默认值：`false`。                                                                                    |
...
| callableStmtCacheSize                   | 如果启用了 `cacheCallableStmts`，则设置驱动程序缓存每个 VM 的 Callable Statement 的数量。默认值：150。     |
...
| clobberStreamingResults                 | 如果在从服务器读取所有数据之前执行另一个查询，这将导致流式结果集自动关闭，并且丢弃当前从服务器进行流式传输但传输未完成的数据。默认值：`false`。 |
| maxRows              | 要返回的最大行数。默认值为 `0`，表示返回所有行。|
| zeroDateTimeBehavior | MySQL 模式下处理无效 Date 的三种方式，有效值为 `convertToNull`、`exception` 或 `round`，即 `ZERO_DATETIME_CONVERT_TO_NULL = "convertToNull";ZERO_DATETIME_EXCEPTION = "exception";ZERO_DATETIME_ROUND = "round";` 默认为 `ZERO_DATETIME_EXCEPTION`。|
| allowNanAndInf       | PreparedStatement.setDouble() 中是否允许使用 NaN 或 +/- INF 值。默认值：`false`。|
| defaultConnectionAttributesBanList | 当 `sendConnectionAttributes=true`时，可以通过该参数来控制不发送到 Server 的 Connection Attribute 的列表，该列表用 “,”分割且区分大小写。默认值：`null`。|
| useInformationSchema     | 是否应该使用 INFORMATION_SCHEMA 来派生 “DatabaseMetaData” 使用的信息。 默认值：`false`。|  
