...
|useArrayBinding  |在 PS 协议下将每个列相同类型的参数拼接成数组的方式，一次 Batch 发送若干个 arrayBinding 包，以提升插入数据的效率。默认值：`false`。<br> **说明** 只在 Oracle 模式下满足 `useServerPrepStmts=true`，且同时使用 `executeBatch()` 或者 `executeLargeBatch()`，且关闭自动提交的时候才会生效，开关起到隔离的作用。</br>|
