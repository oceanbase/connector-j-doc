...
| usePieceData            | 设置是否使用分片数据传输模式。默认值：false。当此参数设置为 true 时，可以在大数据量传输时提高性能。         |
| useArrayBinding         | 设置是否使用数组绑定。默认值：false。当此参数设置为 true 时，可以启用数组绑定，即将多个参数作为数组传递，从而减少与数据库的交互次数。  |
