...
|useArrayBinding  |在 PS 协议下使用将参数绑定为数组的方式，达到一次 Batch 执行多次的目的，减少服务器与客户端的数据交互代价。默认值：`false`。<br> **说明** 只在 Oracle 模式下满足 `useServerPrepStmts=true`，且同时使用 `executeBatch()` 或者 `executeLargeBatch()`，其关闭自动提交的时候才会生效，开关起到隔离的作用。</br>|
