...
可以自动识别 OceanBase 数据库的运行模式是 MySQL 还是 Oracle，并在协议层同时兼容这两种模式。
...
OceanBase Connector/J 驱动，支持标准的 JDBC 应用程序编程接口（API），完全兼容 MySQL 原生的 JDBC 驱动（MySQL Connector Java），OceanBase Connector/J 完全兼容 MySQL JDBC 的使用方式。

- **与Oracle JDBC 兼容性对比**

OceanBase Connector/J 驱动支持标准的 JDBC 应用程序编程接口（API），兼容 Oracle JDBC 的使用方式，兼容其支持的通用接口。
