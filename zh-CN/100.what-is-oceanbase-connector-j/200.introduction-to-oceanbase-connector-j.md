...
OceanBase Connector/J 驱动程序属于 JDBC Type 4 驱动类型，可以通过本地协议直接与数据库引擎通信。OceanBase 数据库支持 OceanBase Connector/J 驱动，同时完全兼容 MySQL 原生的 JDBC 驱动（MySQL Connector Java）。OceanBase Connector/J 完全兼容 MySQL JDBC 的使用方式，可以自动识别 OceanBase 数据库的运行模式是 MySQL 还是 Oracle，并在协议层同时兼容这两种模式。如果您仅访问 MySQL 模式租户，推荐使用社区版 MySQL-JDBC 驱动。更多信息，参考 [MySQL Connector/J 连接 OceanBase 数据库示例程序](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013660)。
